...
if __name__ == "__main__":
    SEGMENTS_DF = pd.read_parquet("results/segments_dedup.parquet")
    ...
    with ThreadPoolExecutor(8) as executor:
        for i in range(0, len(steps), batch):
            batch_steps = steps[i:i+batch]
            executor.map(process_step, batch_steps)
            # This is somewhat nuclear but HF reaaally wants to keep the files on disk.
            print("\n\nCLEANING...\n\n")
            ret = os.system("rm -fr ~/.cache/huggingface")
            assert ret == 0
